	...
	return (
		<SideMenu>
			...
			<section class="h-screen overflow-auto bg-white">
				<div class="mb-6 mt-6 md:flex md:items-center md:justify-between">
					<div>
						<h1>Dashboard</h1>
					</div>
					...
					<div class="divide-x divide-white/5 rounded-lg bg-gray-200/50 text-black md:flex md:items-center md:justify-between">
						<p class="px-4 py-2 text-xs">{dateString}</p>
						<p class="px-4 py-2 text-xs">{timeString}</p>
					</div>
				</div>

				<div
					class="mb-6 mt-6 inline-flex min-w-full overflow-hidden
                rounded-lg shadow-lg backdrop-blur-lg md:flex md:items-baseline md:justify-between"
				>
					<div class="min-w-full p-5 md:flex md:items-baseline">
						<div class="border-gray/50 mr-1 w-1/2 rounded-lg border bg-black">
							<h3 class="m-2 p-2 text-sm font-black text-white">Jumlah Motor Masuk</h3>
							<p class="mb-4 ml-4 mr-4 text-xl font-black text-white">
								{motor?.count} / {motor?.kapasitas}{' '}
							</p>
						</div>

						<div class="border-gray/50 ml-1 w-1/2 rounded-lg border bg-white">
							<h3 class="m-2 p-2 text-sm font-black text-black">Jumlah Mobil Masuk</h3>
							<p class="mb-4 ml-4 mr-4 text-xl font-black text-black">
				...
				<div class="mt-8 flex flex-col">
					<h2 class="mb-4 mt-4"> Kendaraan Masuk Hari Ini</h2>
						...
						<div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
							<div class="overflow-hidden border border-gray-200 dark:border-gray-200 md:rounded-lg">
								<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-200">
									...
									<tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-200 dark:bg-white">
										{filteredData?.map((item) => (
											<tr
												class="text-left text-sm font-normal text-black rtl:text-right"
												key={item.number}
											>
												<td class="px-4 py-3.5 text-right text-sm"> {item?.number} </td>

												<td class="flex items-center gap-x-3 px-4 py-2.5 focus:outline-none">
													<div>
														<p class="text-sm font-bold text-gray-800"> {item?.nomor_kendaraan} </p>
														<p class="text-xs capitalize text-gray-600">{item?.jenis_kendaraan} </p>
													</div>
												</td>

												<td class="whitespace-nowrap px-4 py-3.5 text-sm">
													<div class={item?.kategori_civitas?.toLowerCase()}>
														{item?.kategori_civitas}{' '}
													</div>
												</td>

												<td class="whitespace-nowrap px-4 py-3.5 text-sm"> {item?.waktu_masuk} </td>
												<td class="whitespace-nowrap px-4 py-3.5 text-sm">{item?.waktu_keluar} </td>
											</tr>
										))}
									</tbody>
								</table>
